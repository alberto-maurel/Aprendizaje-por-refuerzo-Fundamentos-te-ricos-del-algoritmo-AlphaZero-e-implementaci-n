...
NUM_ACTIONS = 7
NUM_SIMULATIONS = 200 #Number of simulations per movement
C_PUCT = 1.0          #Constant C_{PUCT} mentioned in the paper
eps = 0.25            #Epsilon to weight the Dirichlet noise
PROF = 8              #Depth at which we are going to make Tau = 0 
                      # (in non training games change it to PROF=0)
DIR_ALPHA = 1.5       #Constant of the Dirichlet noise
...
class MCTS:
    ...
    def __init__(self, probs):
        #When we create a node in our MCTS, we add the Dirichlet noise
    ...
    def simulation(self, board, model):
        ...
        while(True):
            ...
            #Select the action with largest Q + U
    ...
    def makeMove(self, board, model):
        ...
        #Uncomment to print the number of simulations per movement
        #print(list(map(lambda edge: edge.n, self.edges)))

        #Calculate the probability distribution given by the MCTS as the exponentiated
        # visit count of each action. Depending on the current game depth, we use
        # a different TAU hyperparameter
        ...
        return action, copy.deepcopy(board).get_state(), probs, q_val

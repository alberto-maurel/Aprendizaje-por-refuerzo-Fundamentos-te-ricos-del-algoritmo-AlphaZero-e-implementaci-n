...
def main():
    ...
    #Load the models
    model1 = builder.load_model('nn_weights/d3-0.h5')
    model2 = builder.load_model('nn_weights/d3-200.h5')
    ...
    for i in range(0, NUM_PARTIDAS):
        ...
        while not finished and not draw:            
            if board.current_player == 0:
                action, state, mcts_distribution = mcts1.makeMove(board, model1)
            else:
                action, state, mcts_distribution = mcts2.makeMove(board, model2)

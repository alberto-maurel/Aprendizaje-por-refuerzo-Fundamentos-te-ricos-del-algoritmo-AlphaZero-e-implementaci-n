...
class Edge:
    ...
    #Updates an edge once the expansion has taken place. We just need
...
class MCTS:
    ...
    def __init__(self, probs):
        dirich = numpy.random.dirichlet([DIR_ALPHA for _ in range(0,9)], 1)[0]
        self.edges = []
    ...
    def makeMove(self, board, model):
        ...
        #Choose the value of tau depending on the current depth
        if board.current_depth < PROF:
            ...
            probs = [0.0 for _ in range(0,NUM_ACTIONS)]
            probs[action] = 1.0
